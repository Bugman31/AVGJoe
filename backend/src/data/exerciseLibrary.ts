...
export function searchExercises(params: {
  ...
  if (params.muscleGroup) {
    ...
    results = results.filter((ex) => ex.muscleGroups.some((m) => m.toLowerCase().includes(mg)));
  }

  if (params.equipment) {
    const eq = params.equipment.toLowerCase();
    results = results.filter((ex) => ex.equipment.some((e) => e.toLowerCase().includes(eq)));
  }

  if (params.movementPattern) {
    results = results.filter((ex) => ex.movementPattern === params.movementPattern);
  }
