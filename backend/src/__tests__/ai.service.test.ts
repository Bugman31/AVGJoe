...
describe('generateProgram', () => {
  ...
  it('uses the requested total week count when generating a program', async () => {
    (prisma.user.findUnique as jest.Mock).mockResolvedValue({
      anthropicApiKey: null,
      openaiApiKey: null,
      aiProvider: 'anthropic',
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            programName: 'Lean Mass PPL',
            programDescription: 'A longer three-day push pull legs plan.',
            goalSummary: 'Build muscle with a simple PPL split.',
            workoutNotes: [
              { templateName: 'Push Day', coachNote: 'Drive pressing strength.' },
              { templateName: 'Pull Day', coachNote: 'Own each pull.' },
    ...
    const program = await generateProgram('user-1', undefined, 6);

    expect(program.totalWeeks).toBe(6);
    expect(program.workouts).toHaveLength(18);
    expect(program.workouts.filter((workout) => workout.weekNumber === 6)).toHaveLength(3);
    expect(program.workouts.some((workout) => workout.name.includes('Deload'))).toBe(true);

    const prompt = mockAnthropicCreate.mock.calls[0][0].messages[0].content;
    expect(prompt).toContain('Requested program length: 6 weeks');
  });
});
